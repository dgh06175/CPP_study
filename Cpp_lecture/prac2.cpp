...
{
    Circle_F waffle(30);
    increase(waffle);                   // 객체의 참조를 전달
    cout << waffle.getRadius() << endl; // 이제 31을 출력할 것입니다.
}
...
public:
    int getColorSum();
    Color sum(Color &left, Color &right);
    // Color operator+(Color &left, Color &righr);
};
...
public:
    friend int getColorsum();
    friend Color ColorsManagement::sum(Color &left, Color &right);
    // friend Color operator+(Color left, Color right);

    Color operator+(Color in)
    {
        return Color(this->r + in.r, this->g + in.g, this->r + in.r);
    }
...
Color ColorsManagement::sum(Color &left, Color &right)
{
    Color color(left.r + right.r, left.g + right.g, left.b + right.b);
    return color;
}
...
void FINAL_3()
{
    ...
    Color color3;
    ColorsManagement colorsmanagement;
    // color3 = colorsmanagement.sum(color1, color2);
    color3.show();
    Color color4 = color1 + color3;
    color4.show();
}
...
public:
    ...
    void show();
    void operator+=(Power op2);
    Power operator++(); // 전위 ++ 연산자 함수 선언
    Power operator++(int x);
    friend Power operator+(int op1, Power op2); // 프렌드 선언
    friend Power operator+(Power op1, int op2); // 프렌드 선언
};

void Power::operator+=(Power op2)
{
    kick = kick + op2.kick;
...
{
    Power tmp = *this; // 증가 이전 객체 상태를 저장 kick++;
    punch++;
...
{
    Power tmp;                 // 임시 객체 생성
    tmp.kick = op1 + op2.kick; // kick 더하기 tmp.punch = op1 + op2.punch; // punch 더하기 return tmp; // 임시 객체 리턴
...
{
    Power tmp;                 // 임시 객체 생성
    tmp.kick = op2 + op1.kick; // kick 더하기 tmp.punch = op1 + op2.punch; // punch 더하기 return tmp; // 임시 객체 리턴
    tmp.punch = op2 + op1.punch;
    return tmp;
}
